...
function update_ticket_container(container_id, list, new_id, is_active) {
    ...
    if (list.length === 0) {
        const msg_text = is_active ? "<Aucun ticket en cours>" : "<Aucun ticket terminé>";
        const msg = create_tag('div', 'empty-message', msg_text);
    ...
    list.forEach(t => {
        ...
        if (is_active) {
            ...
            div.innerHTML = `
                ...
                <div class="time"><p id="created">${time_str}</p><p id="remaining">${format_time_elapsed(t.dateCreation)}</p></div>
                ${delete_btn}
            `;
        } else {
            div.innerHTML = `
                <p class="name">${t.nom}</p>
                <div class="time"><p class="created">${time_str}</p><p class="etat">${t.etat}</p></div>
                ${delete_btn}
            `;
        }
...
function render_announcements() {
    ...
    if (left_container) {
        left_container.style.gap = announcements_list.length === 0 ? '0px' : '';
    }
    ...
    announcements_list.forEach((annonce, index) => {
        ...
        if (annonce.content?.trim()) {
            ...
            if (is_admin) {
                text_row.querySelector('.announcement-delete').addEventListener('click', (e) => delete_item(e, `/api/announcements/${annonce.id}`, wrapper));
            }
    ...
    pending_files.forEach((file, index) => {
        ...
        const item = create_tag('div', 'admin-file-item', `
            ...
            <div class="admin-file-info">
                ...
                <span class="admin-file-size">${file_size} Mo</span>
...
async function process_admin_upload(content, color) {
    ...
    try {
        ...
        if (e.message !== "Aborted") {
            ...
            render_pending_files(); // reset ui
        }
    } finally {
        is_sending = false;
        current_xhr = null;
        if (create_btn) {
            create_btn.classList.remove('button-disabled');
        }
    }
}
...
function setup_event_listeners() {
    ...
    document.querySelectorAll('input[name="SliderCount"]').forEach(radio => {
        radio.addEventListener('change', async (e) => {
            ...
            max_tickets = val;
            if (is_admin) {
                await api_call(`/api/rooms/${room_code}`, "PUT", { maxTickets: val });
            }
        });
    });
...
function setup_drag_and_drop() {
    ...
    dropArea.onclick = () => {
        if (pending_files.length >= max_files) {
            return alert("Limite atteinte.");
        }
        fileInput.click();
    };
